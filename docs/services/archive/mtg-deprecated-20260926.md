> ⚠️ **ДЕМОНТИРОВАНО 26.09.2026.** MTProxy полностью удалён с S1: сервисы
> остановлены, `/etc/mtg` удалён, iptables/policy routing очищены,
> пользователь `mtproxy` удалён. Причина: не работал во время реальных
> блокировок (timeout к fronting-доменам), признан нежизнеспособным.
> Файл сохранён только как историческая справка. См. `system-state.md`.
